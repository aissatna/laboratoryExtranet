...
if (isset($_POST['add-provider'])) {
    ...
    $req_fields = array('provider-companyName', 'provider-email','provider-siteweb');
    validate_fields($req_fields);
    $file = find_by_field('fournisseurs', $fileName, 'ListeDesPrix');
    if (empty($file) or $fileName === "") {
        if (empty($errors)) {
            $company_name = remove_junk($db->escape($_POST['provider-lastname']));
            $email = remove_junk($db->escape($_POST['provider-email']));
            $web_site = remove_junk($db->escape($_POST['provider-siteweb']));

            $phone = (empty($_POST['provider-phone'])) ?
